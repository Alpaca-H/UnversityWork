...
class  Sc(scrapy.Item):
    datatime = scrapy.Field()
    startTime = scrapy.Field()
    matchName = scrapy.Field()
    link = scrapy.Field()
    leftLogo = scrapy.Field()
    leftName = scrapy.Field()
    rightLogo = scrapy.Field()
    rightName = scrapy.Field()
    vsLine = scrapy.Field()

    def get_insert_sql(self):
        insert_sql = """
            insert into saicheng(datatime,startTime,matchName,link,leftLogo,leftName,rightLogo,rightName,vsLine)
            VALUES (%s, %s, %s,%s,%s, %s, %s,%s,%s)
            ON DUPLICATE KEY UPDATE link=VALUES(link)
        """

        params = (
            self["datatime"], self["startTime"], self["matchName"],
            self["link"], self["leftLogo"], self["leftName"],
            self["rightLogo"], self['rightName'],self['vsLine']
        )
